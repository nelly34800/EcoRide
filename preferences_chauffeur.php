...
$preferences = getPreferences($pdo, $user_id); // Récupérer l'ID des préférences
...
<div class="container">
    ...
    <?php if (!empty($preferences)): ?>
        <p class="card-text">J'accepte les animaux: <?= choice($preferences['pets']); ?></p>
        <p class="card-text">J'accepte de faire des pauses pour les fumeurs: <?= choice($preferences['smoking']); ?></p>
        <p class="card-text">Autres préférences: <?= htmlspecialchars($preferences['others'] ?: 'Aucune'); ?></p>
        <div class="m-2">
            <a href="ajout_preferences_chauffeur.php?id=<?= $preferences['id']; ?>" class="btn btn-primary m-2">Modifier vos préférences</a>
            <a href="sup_preferences.php?id=<?= $preferences['id']; ?>" class="btn btn-dark m-2" onclick="return confirm('Êtes-vous sûr de vouloir supprimer vos préférences ?');">Supprimer vos préférences</a>
        </div>
    <?php else: ?>
